# widget-package Specification

## Purpose
Defines how widgets are packaged and discovered on disk: a `widgets/` directory beside the
executable, each subfolder a single widget identified by its folder name, declared by a
`widget.toml` manifest and pointing at a Lua entry file the app reads before execution.
## Requirements
