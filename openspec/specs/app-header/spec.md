...
## Purpose
Defines the persistent app-level header that frames the hosted tool: window-edge padding,
the app name on the left, and a right-aligned action group (a placeholder Configs button and
a link to the project's GitHub page). The header is independent of whichever tool the host
renders below it.
## Requirements
