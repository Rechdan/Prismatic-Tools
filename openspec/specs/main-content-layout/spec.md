...
## Purpose
Defines the window body layout: a padded, persistent two-pane main region (a fixed-width
left navigation column plus a flexible right container) that fills the whole window with no
header row above it, and the explicit selection between the active views the right container
can show (the home view by default, the active widget, or a placeholder config view). The
left navigation column pins the app title at the top and a Configs/GitHub action group at
the bottom, and scrolls a middle region that surfaces a Home entry and the loaded widget as
selectable entries, highlighting the active view's entry.
