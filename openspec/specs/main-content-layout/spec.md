...
selection between the active views the right container can show (the active widget by
default, or a placeholder config view). The left navigation column surfaces the loaded
widget as a selectable entry so the user can return to the widget view after opening config.
