...
## Purpose
Defines the shell's **home view**: a read-only landing page that renders the
project `README.md` (bundled into the executable at build time) as formatted
markdown inside the right container. It maps block-level markdown onto the shell's
UI elements, flattens inline runs to readable text, renders links as plain text,
keeps the content selectable but not editable, scrolls vertically for long
documents, and degrades unhandled constructs to text rather than crashing.
