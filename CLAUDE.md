...
- **`src/shell.rs`** — the single-instance guard, tray, and rendered tool surface.
  ...
  - **Layout / Nav** — the window body is reactor's native `NavigationView` (see `docs/reactor-notes.md` → "NavigationView — the shell's navigation"): a Left-mode, burger-toggled (`pane_toggle_button_visible`) pane titled `Prismatic Tools`, holding icon+label `NavViewItem`s — `Home` (`Symbol::Home`), the loaded widget (its manifest **name**, `Symbol::Document`), and a **Configs** item (`Symbol::Setting`) last. Configs is a **normal tagged menu item**, not the built-in Settings gear — the gear is disabled (`settings_visible(false)`) because its selection does not route through reactor (runtime-confirmed: clicking it fell through to home). A widget that fails to load appears instead as a selectable error item (`Symbol::Important`, labeled with its folder id); an absent widget adds no item. There is no custom nav card, no hover/selection fill overlays, and **no GitHub item** (dropped — the README home view keeps the link; a browser-opening item can't cleanly un-highlight itself, see reactor-notes). The active view renders in the NavigationView content area, wrapped once in a stretched `.padding(16)` `border` (the inset — the stretch keeps the home `scroll_viewer` bounded).
  - **View state** — selection is a single `use_state::<String>` **tag** (default `"home"`) echoed to `.selected_tag(..)` and updated by `.on_selection_changed(set_tag)` (the `SetState` setter is passed directly). Tag namespaces route the content area, each a real (selectable+highlightable) menu item: `"home"` → the README home view (`src/home.rs`); `"widget:<id>"` → the widget view (its render, or the load-error text); `"config"` → the config view. `""` is a deselection artifact (reactor rebuilds a changed `menu_items` via `menu.Clear()`, deselecting a live MenuItem → spurious `""`), also routed to config. **Menu-rebuild invariant:** the only `menu_items` mutator (Reload) is reachable solely from the config view, so the only item selected during a rebuild is Configs itself — mapping `"" ⇒ config` keeps content stable; don't add a menu-mutating action reachable from Home or the widget view.
...
- **`src/widget/`** — the Lua widget runtime (the tool mechanism). Tools live under `widgets/` beside the exe, one folder per widget, each with a `widget.toml` manifest (`name` required; `version`/`entry` optional, `entry` default `main.lua`; the folder name is the widget id).
  ...
  - **Contract** — the chunk returns a `{ state, render }` table: `state` is a plain-data table the host holds (source of truth, kept for future persistence); `render(state)` returns a declarative UI table (`vstack`/`hstack`/`text`/`button`/`border`; hash keys = props, array entries = children) mapped to reactor elements each render. The `border` node wraps one child (multiple children → implicit `vstack`). The widget is surfaced in navigation by its manifest **name** only — there is no widget-drawn nav preview (`NavViewItem` content is string-only; `LoadedWidget::id()` is the routing key, `name()` the label).
  ...
  - **Shipped** — `widgets/counter/` is the first widget (the ported demo counter).
